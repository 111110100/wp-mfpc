             ...
             <?php if ( isset( $_GET['settings-updated'] ) ) : ?>
                <div id="setting-error-settings_updated" class="notice notice-success settings-error is-dismissible">
                    ...
                    <?php if ( get_transient('mfpc_nginx_config_success') ) : ?>
                        ...
                        <p><?php printf(
                                wp_kses_post( __( 'You need to include this file in your main Nginx configuration (e.g., using an `include %s;` directive) and reload Nginx.', 'mfpc-config' ) ),
...
function mfpc_sanitize_settings( $input ) {
    ...
    if ( file_exists( MFPC_NGINX_TEMPLATE_FILE_PATH ) ) {
        ...
        if ( $template_content !== false ) {
            $upstream_block = "\n# Auto-generated by Memcached Full Page Cache Config plugin\n";
            ...
            $nginx_config_content = $template_content . $upstream_block;
            $write_nginx_result = file_put_contents( MFPC_NGINX_OUTPUT_FILE_PATH, $nginx_config_content );

            if ( $write_nginx_result === false ) {
                $error_msg = sprintf( __( 'Error: Could not write Nginx configuration file to %s. Please check file permissions.', 'mfpc-config' ), MFPC_NGINX_OUTPUT_FILE_PATH );
                ...
                set_transient('mfpc_nginx_config_success', sprintf( __( 'Nginx config file generated successfully at %s.', 'mfpc-config' ), MFPC_NGINX_OUTPUT_FILE_PATH ), 60);
